...
import FootballApi, { BASE_URL_FOOTBALL_API } from "src/data/api/FootballApi"

customElements.define("nav-navigation", class Navigation extends BaseCustomHtml {
    constructor() {
        super()
        caches.match(`${BASE_URL_FOOTBALL_API}/v2/competitions/2001/standings`).then((response) => {
            if (response === undefined) new FootballApi().getData(FootballApi.URL_STANDINGS()).then()
        })
        caches.match(`${BASE_URL_FOOTBALL_API}/v2/competitions/2001/teams`).then((response) => {
